...
const ToolTip = ({infoText="This is a tooltip", className, children="Hover me"} : {infoText?: string, className?: string, children?: React.ReactNode}) => {
  return (
      <button className={cn("relative cursor-pointer text-md group scale-97 hover:scale-100 transition-transform duration-200 active:scale-98 font-inter text-[14px]", className)}>
        {children}
